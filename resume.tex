...
\section{Skills}
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
	\item \textbf{Languages & Platforms}: Dart, Kotlin, Swift, SwiftUI, Java, Objective-C
	\item \textbf{Mobile Development}: iOS (MVC, MVVM, MVP, VIPER), Android (MVC, MVVM), Flutter (Riverpod, Provider, VIPER)
	\item \textbf{Networking & Communication}: REST API, WebSocket, SignalR, Dio, Retrofit
	\item \textbf{Databases}: Drift, Room, SQLite, Hive
	\item \textbf{Backend}: Node.js (Express), MongoDB, JSP
	\item \textbf{Build & Dependency Management}: Gradle, CocoaPods, SPM, npm
	\item \textbf{Localization}: i18n, JSON-based dynamic language support
	\item \textbf{Error Tracking & Logging}: Sentry, Local Logging, SignalR Logs
	\item \textbf{Design Patterns}: MVC, MVVM, MVP, VIPER, Singleton, Factory
	\item \textbf{Architectures}: Clean Architecture, Feature-Based Architecture, Monolithic Architecture
